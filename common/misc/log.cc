...
void Log::initFileDescriptors()
{
   ...
   _loggerFiles = new FILE*[LogCustom::END-LogCustom::START+1];
   _loggerLocks = new Lock[LogCustom::END-LogCustom::START+1];
   for(int i=LogCustom::START; i<= LogCustom::END; i++)
   {
      _loggerFiles[i] = NULL;
   }
...
void Log::log(Log::LogCustom log_custom, const char *format, ...)// Log::LogState logState,
{
   FILE *file = NULL;
   Lock *lock = NULL;
   ...
   if(_loggerFiles[log_custom]==NULL)
   {
      sprintf(filename, "customLog_%u.log", log_custom);
      _loggerFiles[log_custom]=fopen(formatFileName(filename).c_str(), "w");
   }
   file = _loggerFiles[log_custom];
   if(sim_thread)
      lock = &_simLocks[core_id];
   else
      lock = &_coreLocks[core_id];
